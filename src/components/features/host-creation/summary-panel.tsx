...
import {
  ...
  removeStop,
  reorderStop,
} from "@/store/host-creation-slice";
...
export function SummaryPanel({ draftId }: SummaryPanelProps) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id) {
      dispatch(
        moveStop({ activeId: active.id as string, overId: over?.id as string }),
      );
    }
  };
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (!saveResult.success) {
        throw new Error(
          saveResult.error || "Failed to sync draft before publishing",
        );
      }
  ...
  return (
    <div className="w-full min-w-[320px] h-full bg-[var(--background)] border-r border-[var(--border)] overflow-y-auto flex flex-col pointer-events-auto">
      ...
      <div className="flex-1 p-6 space-y-8">
        {/* 1. Host Info */}
        <section>
          <h3 className="text-xs uppercase tracking-wide font-semibold text-[var(--muted-foreground)] mb-3">
            Host
          </h3>
                    ...
                    <div className="p-3 bg-[var(--muted)]/30 border-b border-[var(--border)] flex items-start gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() =>
                            dispatch(
                              reorderStop({ id: stop.id, direction: "up" }),
                            )
                          }
                          disabled={idx === 0}
                        ...
                        <button
                          onClick={() =>
                            dispatch(
                              reorderStop({ id: stop.id, direction: "down" }),
                            )
                          }
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                value={shortDesc || ""}
                onChange={(e) =>
                  dispatch(updateDraft({ shortDesc: e.target.value }))
                }
            ...
            <div>
              ...
              <textarea
                value={longDesc || ""}
                onChange={(e) =>
                  dispatch(updateDraft({ longDesc: e.target.value }))
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  dispatch(
                    updateDraft({ duration: parseInt(e.target.value) || 0 }),
                  )
                }
                placeholder="e.g. 180"
                className="w-full px-3 py-2 bg-[var(--card)] rounded-lg border border-[var(--border)] text-sm focus:ring-2 focus:ring-[var(--princeton-orange)] focus:border-transparent outline-none transition-all placeholder:text-[var(--muted-foreground)]/50"
      ...
      <div className="p-4 border-t border-[var(--border)] bg-[var(--background)]">
        <button
          disabled={stops.length === 0 || isPublishing || !draftId}
