...
export function __setUnsplashRouteExternalApiCallerForTests(
  caller: ExternalApiCaller | null
): void {
  externalApiCaller = caller ?? callExternalApi;
}

export function __clearUnsplashRouteCacheForTests(): void {
  responseCache.clear();
}
...
async function searchUnsplash(query: string, accessKey: string, page: number) {
  ...
  try {
    const response = await externalApiCaller({
      provider: 'UNSPLASH',
      endpoint: 'unsplash.searchPhotos.redirect',
      url: apiUrl.toString(),
      method: 'GET',
      headers: {
...
export async function GET(request: Request) {
  ...
  const cacheKey = getCacheKey(query, width, height, sig);
  ...
  const index = sig > 0 ? sig % candidates.length : 0;
