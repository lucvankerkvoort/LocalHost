...
export async function createTrip(data: CreateTripData) {
  ...
  const resolvedCity = hasCity ? trimmedCity : null;

  // Resolve coordinates only when we have a city
  let lat = 0;
  let lng = 0;
  if (hasCity) {
    // 1. Try static map
    const staticCoords = getCityCoordinates(trimmedCity);
    if (staticCoords) {
      ...
      try {
        const geoCoords = await geocodeCity(trimmedCity);
        if (geoCoords) {
          lat = geoCoords.lat;
          lng = geoCoords.lng;
        }
      } catch (e) {
        console.warn(`Failed to geocode city: ${trimmedCity}`, e);
      }
    }
  }

  try {
    const tripData: Parameters<typeof prisma.trip.create>[0]['data'] = {
      userId: session.user.id,
      title: title?.trim() || (resolvedCity ? `Trip to ${resolvedCity}` : 'New Trip'),
    ...
    const newTrip = await prisma.trip.create({ data: tripData });
